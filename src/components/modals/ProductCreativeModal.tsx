...
export default function ProductCreativeModal({
  ...
  const originalUrl = useAssetUrl((product?.image ?? {}) as ImageReference)
  ...
  const save = async () => {
    ...
    try {
      ...
      applyToProduct(product.id, stored.id, name)
      setMessage('La fotografía mejorada se guardó y ya aparece en el catálogo.')
    } catch (caught) {
      setError(
  ...
  return (
    <Modal
      ...
      title={`Estudio creativo · ${product.name}`}
      description="Mejora o reemplaza la fotografía de este producto sin salir de su ficha."
      size="xl"
      footer={
        <>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="overflow-hidden rounded-2xl border border-border bg-slate-100">
          <div className="flex items-center justify-between gap-3 border-b border-border bg-white p-4">
            ...
            <Badge tone="primary">Edición no destructiva</Badge>
          </div>
